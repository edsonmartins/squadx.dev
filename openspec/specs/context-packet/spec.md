...
Verdade atual. Input curado e auditável ao executor, em vez de concatenação solta de contexto.
Realiza ADR-0007, RFC-0005 (handoff). Materializado a partir de `changes/agent-work-governance`.
...
construído de forma estável (uma vez por execução de subtask, não recomputado por leitura).
...
- **AND** é passado ao agente junto do contexto bruto
