...
public class Pass5Service {
    ...
    public Pass5Run validate(Long specTaskId, String prSha) {
        ...
        if (total == 0) {
            ...
            if (!uncovered.isEmpty()) {
                ...
                if (prSha != null) {
                    var testResult = testSuiteExecutor.execute(specTaskId, prSha);
                    if (!testResult.passed()) {
                        outcome = Pass5Result.FAIL;
                        critique = "Testes mapeados falharam: " + testResult.summary();
                        return persistOutcome(task, prSha, outcome, critique, total, covered);
                    }
                }
    ...
    private Pass5Run persistOutcome(SpecTask task, String prSha, Pass5Result outcome, String critique,
        ...
        specTaskService.applyPass5Outcome(task.getId(), outcome, critique);     // R4
        return run;
    }
    ...
    private ConformanceReviewer.ConformanceRequest toRequest(Long specTaskId, String prSha,
        ...
        List<ConformanceReviewer.ScenarioRef> refs = scenarios.stream()
                ...
                .collect(Collectors.toList());
        String diff = prSha == null ? null : gitDiffProvider.diff(specTaskId, prSha);
        return new ConformanceReviewer.ConformanceRequest(specTaskId, prSha, refs, diff);
    }
