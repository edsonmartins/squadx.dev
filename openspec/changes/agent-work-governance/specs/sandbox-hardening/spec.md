# Spec (delta) — sandbox-hardening

Reduzir vazamento de segredo e poluição de commits no caminho de execução. Realiza ADR-0007
(espelha `opentag/packages/runner/src/security.ts` e `git.ts`).
...
(`enforce | audit | off`). Em `audit`, SHALL registrar um evento com `visibility = "audit"`; em
`enforce`, SHALL bloquear a execução e escalar.
