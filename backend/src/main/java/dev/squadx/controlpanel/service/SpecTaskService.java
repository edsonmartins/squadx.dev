...
import dev.squadx.controlpanel.dto.spectask.SpecEventResponse;
...
public class SpecTaskService {
    ...
    @Transactional(readOnly = true)
    public List<SpecEventResponse> getEvents(Long taskId, User currentUser) {
        loadForUser(taskId, currentUser);
        return specEventRepository.findBySpecTaskIdOrderByOccurredAtAscIdAsc(taskId).stream()
                .map(event -> SpecEventResponse.builder().id(event.getId()).type(event.getType())
                        .source(event.getSource()).sourceRef(event.getSourceRef()).payload(event.getPayload())
                        .occurredAt(event.getOccurredAt()).receivedAt(event.getReceivedAt()).build())
                .toList();
    }
